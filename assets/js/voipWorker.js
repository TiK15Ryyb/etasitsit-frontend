        ...
        if (inc) {
            ...
            if (inDataArrayBuffer != null) {
                resample(
                    ...
                    outSampleRate,
                    outChunkSize,
                    function(resapledData) {
                        if (inBitrate == 8) {
                            resapledData = mapUint8ToFloat32Array(resapledData);
                        } else if (inBitrate == 16) {
                            resapledData = mapUint16ToFloat32Array(
                                resapledData
                            );
                        }
                        var lowDat = lowSignal(resapledData, p);
                        self.postMessage([clientId, lowDat, x, y]); //Send data back....
                    }
                );
            } else {
            ...
            //Data from the client itself (Add encoding information before send it to the server)
            resample(
                inDataArrayBuffer,
                inSampleRate,
                outSampleRate,
                ...
                function(resapledData) {
                    ...
                    if (resapledData != null) {
                        if (outBitRate == 8) {
                            ...
                            bitratedData = mapFloat32ToUInt16Array(
                                resapledData
                            );
                        }

                        self.postMessage([bitratedData, p, x, y]); //Send it back....
                    }
                }
            );
        }
...
function resample(
    ...
    if (inSampleRate > outSampleRate) {
        doneCallback(
            downSample(inSampleRate, outSampleRate, inDataArrayBuffer)
        );
    } else {
        upSample(
            inSampleRate,
...
function maxSignal(buffer) {
    ...
    if (buffer) {
        for (var i = 0; i < buffer.length; i++) {
            if (max < Math.abs(buffer[i])) max = Math.abs(buffer[i]);
        }
    }
  ...
  */
    ...
    LinearInterpolator = (function(_super) {
        ...
        LinearInterpolator.prototype.interpolate = function(t) {
            ...
            t -= k;
            return (
                (1 - t) * this.getClippedInput(k) +
                t * this.getClippedInput(k + 1)
            );
        };
    ...
    CubicInterpolator = (function(_super) {
        ...
        function CubicInterpolator(array, config) {
            this.tangentFactor =
                1 - Math.max(0, Math.min(1, config.cubicTension));
            ...
            switch (getType(arr[0])) {
                ...
                case "Array":
                    ...
                    return function(t) {
                        ...
                        _results = [];
                        for (
                            _k = 0, _len3 = interpolators.length;
                            _k < _len3;
                            _k++
                        ) {
